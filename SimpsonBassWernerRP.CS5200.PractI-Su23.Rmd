...
```{sql question8query, connection=dbcon, eval=T}
...
```{sql question9query, connection=dbcon, eval=T}
...
```{r question10query, eval = T, warning = F}
library(DBI)

query <- "
  SELECT DATE_FORMAT(date, '%Y-%m') AS month, SUM(numbirds) AS total_birds
  FROM strikes
  JOIN flights ON strikes.fid = flights.fid
  GROUP BY month
  ORDER BY month;
